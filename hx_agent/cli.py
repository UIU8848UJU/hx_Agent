...
# 配置文件和日志的头文件
from hx_agent.app_context import Path, ensure_default_config, get_ctx, settings
...
def doctor():
    ...
    try:
        ...
        print(
            f'  notes: scanned={n.get("scanned")} rebuilt_files={n.get("rebuilt_files")} skipped={n.get("skipped")} inserted_chunks={n.get("inserted_chunks")} failed={len(n.get("failed") or [])} elapsed_sec={n.get("elapsed_sec")}'
        )
    except Exception:
